...
classApp.controller('weatherCtrl', function($scope, $http) {
    ...
    $http.get("http://ip-api.com/json").success(function(data) {
        ...
        $http.get(openWeatherUrl).success(function(data) {
            vm.description = data.list[7].weather[0].description;
            // console.log(data.list[0].weather[0].description);
            vm.date = (data.list[7].dt*1000);
            vm.joke = data.city.name;
            vm.speed = (2.237 * data.list[7].wind.speed).toFixed(1) + " mph";
            vm.temp = data.list[7].main.temp;
            vm.fTemp = (vm.temp * (9 / 5) - 459.67).toFixed(1) + "F ";
            vm.cTemp = (vm.temp - 273).toFixed(1) + "C ";
            vm.icon = "http://openweathermap.org/img/w/" + data.list[7].weather[0].icon + ".png";
